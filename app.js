...
Promise.all([fetch('data/search-index.json?v=karma-18-20260816', { cache: 'no-store' }).then(r => r.json()), fetch('data/matching-rules.json?v=karma-18-20260816', { cache: 'no-store' }).then(r => r.json()), fetch('data/guide-manifest.json?v=karma-18-20260816', { cache: 'no-store' }).then(r => r.json())]).then(([data, loadedRules, catalogue]) => { records = data.records; keywordRecords = [...new Map([...builtInKeywordRecords, ...(data.keyword_records || []), ...(catalogue.guides || []).map(catalogueRecord)].map(item => [`${item.model}|${item.guide}`, item])).values()]; rules = loadedRules; indexReady = true; searchButton.disabled = false; status.textContent = `Search ${records.length.toLocaleString()} indexed model locations and ${(catalogue.guides || []).length.toLocaleString()} current public guides from ${data.updated}.`; if (input.value.trim()) search(); }).catch(() => { keywordRecords = builtInKeywordRecords; indexReady = true; searchButton.disabled = false; status.textContent = 'Product-category search is available. The full model index could not be loaded.'; if (input.value.trim()) search(); });
