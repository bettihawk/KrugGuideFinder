...
function card(item, close) {
  ...
  return `<article class="card"><div><h3>${item.guide}</h3><p class="model">${item.model}${match}</p><p class="meta">${item.market} · Guide page ${guidePage}</p></div><div class="actions"><a href="${viewerUrl}" target="_blank" rel="noopener">View guide page ${guidePage} ↗</a><a class="source-link" href="${pageUrl}" target="_blank" rel="noopener">Original PDF</a></div></article>`;
}
...
Promise.all([fetch('data/search-index.json?v=printed-pages-20260808').then(r => r.json()), fetch('data/matching-rules.json?v=printed-pages-20260808').then(r => r.json())]).then(([data, loadedRules]) => { records = data.records; rules = loadedRules; status.textContent = `Search ${records.length.toLocaleString()} indexed model locations from ${data.updated}.`; }).catch(() => status.textContent = 'The search index could not be loaded.');
