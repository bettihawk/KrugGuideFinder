...
Promise.all([fetch('data/search-index.json?v=full-catalogue-20260816b', { cache: 'no-store' }).then(r => r.json()), fetch('data/matching-rules.json?v=full-catalogue-20260816b', { cache: 'no-store' }).then(r => r.json()), fetch('data/guide-manifest.json?v=full-catalogue-20260816b', { cache: 'no-store' }).then(r => r.json())]).then(([data, loadedRules, catalogue]) => { records = data.records; keywordRecords = [...new Map([...builtInKeywordRecords, ...(data.keyword_records || []), ...(catalogue.guides || []).map(catalogueRecord)].map(item => [`${item.model}|${item.guide}`, item])).values()]; rules = loadedRules; status.textContent = `Search ${records.length.toLocaleString()} indexed model locations and ${(catalogue.guides || []).length.toLocaleString()} current public guides from ${data.updated}.`; }).catch(() => { keywordRecords = builtInKeywordRecords; status.textContent = 'Product-category search is available. The full model index could not be loaded.'; });
