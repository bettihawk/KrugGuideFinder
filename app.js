...
Promise.all([fetch('data/search-index.json?v=comprehensive-guides-20260816', { cache: 'no-store' }).then(r => r.json()), fetch('data/matching-rules.json?v=comprehensive-guides-20260816', { cache: 'no-store' }).then(r => r.json())]).then(([data, loadedRules]) => { records = data.records; keywordRecords = [...new Map([...builtInKeywordRecords, ...(data.keyword_records || [])].map(item => [`${item.model}|${item.guide}`, item])).values()]; rules = loadedRules; status.textContent = `Search ${records.length.toLocaleString()} indexed model locations and ${keywordRecords.length.toLocaleString()} product categories from ${data.updated}.`; }).catch(() => { keywordRecords = builtInKeywordRecords; status.textContent = 'Product-category search is available. The full model index could not be loaded.'; });
